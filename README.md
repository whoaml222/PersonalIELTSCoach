...
- 100 个内置高频词、三种新词练习、在线英音词典发音与本地音频缓存
- 1 / 3 / 7 天复习排程与错词本
...
首次安装后先完成水平测试。当前版本为 `1.5.0`。只有使用 AI 句子分析或写作批改时才需要在“设置”中填写 OpenAI API Key；英文朗读使用在线英音词典，不需要 API Key。默认分析模型为 `gpt-5.4-mini`，也可选择 `gpt-5.4-nano` 或 `gpt-5.4`。
